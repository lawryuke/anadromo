Shader "Anadromo/OceanViz Rock Massif"
{
    Properties
    {
        ...
        _BaseMap("OceanViz scanned rock", 2D) = "white" {}
        [Normal] _BumpMap("OceanViz rock normals", 2D) = "bump" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 RockFragment(Varyings input) : SV_Target
            {
                ...
                float3 rockX=SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,p.zy*.3).rgb;
                float3 rockY=SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,p.xz*.3).rgb;
                float3 rockZ=SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,p.xy*.3).rgb;
                float3 scanned=rockX*weights.x+rockY*weights.y+rockZ*weights.z;
                surface.albedo = scanned * _BaseColor.rgb * lerp(.8,1.2,macro);
                ...
                float3 tx=UnpackNormal(SAMPLE_TEXTURE2D(_BumpMap,sampler_BumpMap,p.zy*.3));
                float3 ty=UnpackNormal(SAMPLE_TEXTURE2D(_BumpMap,sampler_BumpMap,p.xz*.3));
                float3 tz=UnpackNormal(SAMPLE_TEXTURE2D(_BumpMap,sampler_BumpMap,p.xy*.3));
                float3 scannedNormal=float3(tx.z*sign(n.x),tx.y,tx.x)*weights.x
                    +float3(ty.x,ty.z*sign(n.y),ty.y)*weights.y
                    +float3(tz.x,tz.y,tz.z*sign(n.z))*weights.z;
                lighting.normalWS = normalize(lerp(n,scannedNormal,.6) - gradient * _BumpScale*.4);
